...
class PathSecurity {
  ...
  static bool isSafePath(String path) {
    ...
    return riskLevel == PathRiskLevel.safe || riskLevel == PathRiskLevel.warning;
  }
  ...
  /// 获取路径风险的描述信息
  static String getPathRiskDescription(String path, {required String operation}) {
    final riskLevel = getPathRiskLevel(path);
  ...
  static void logOperation({
    ...
    if (allowed) {
      ...
      logger.w('[$status] $operation: "$pathName" (Risk: ${riskLevel.name})${reason != null ? ' - $reason' : ''}');
    }
    
    // 对于被阻止的危险操作，记录完整路径用于审计
    if (!allowed && (riskLevel == PathRiskLevel.danger || riskLevel == PathRiskLevel.forbidden)) {
