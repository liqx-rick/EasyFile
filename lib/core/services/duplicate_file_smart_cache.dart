...
class DuplicateFileSmartCache {
  ...
  Future<DuplicateFileScanCache?> loadCache(DuplicateFileScanConfig config) async {
    try {
      ...
      if (jsonStr == null) {
        logger.d('No cache found for config');
        return null;
      }
      ...
      if (!cache.config.isEquivalent(config)) {
        logger.w('Cache config mismatch, ignoring cache');
        return null;
      }
      
      // 检查是否过期
      if (cache.isExpired()) {
        logger.w('Cache expired, ignoring');
        return null;
      }
      
      logger.i('Cache loaded successfully (${cache.groups.length} groups, age: ${DateTime.now().difference(cache.scanTime).inHours}h)');
      return cache;
  ...
  List<DuplicateFileGroup> applyIncrementalUpdate(
    ...
    // 从缓存复制所有组
    final groups = cache.groups.map((g) => DuplicateFileGroup(
      groupId: g.groupId,
